...
export async function getAllToken() {
  try {
    ...
    for (const token of allTokens) {
      ...
      try {
        const projectResult = await db
          .select()
          .from(ProjectTable)
          .where(eq(ProjectTable.id, idProjek))
          .limit(1);
        project = projectResult[0] || null;
      } catch (projectError) {
        console.error(
          `Error fetching project with ID ${idProjek}:`,
          projectError
        );
      }
    ...
    console.error(
      "Error in fetching tokens from blockchain and database:",
      error
    );
    throw error;
  }
...
export async function getTotalToken() {
  try {
    ...
    console.error(
      "Error in fetching total token count from blockchain:",
      error
    );
    throw error;
  }
...
export async function getTokenByIdProject(id: string) {
  try {
    ...
    console.log(
      "Fetched token by project ID from blockchain:",
      serializedTokens
    );
    ...
    console.error(
      "Error in fetching token by project ID from blockchain:",
      error
    );
    throw error;
  }
...
export async function getTokenProjectByUser(
  id_user: string,
  id_projek: string
) {
  try {
...
async function processUpdateQueue() {
  ...
  try {
    ...
    while (updateTokenQueue.length > 0) {
      console.log(
        "Processing token update transaction with nonce",
        currentNonce
      );
...
export async function buyTokenProject(
  id_user: string,
  id_projek: string,
  jumlah_token: number
) {
  ...
  const ownerProject = await db
    .select()
    .from(UserTable)
    .where(eq(UserTable.id, project[0].id_user))
    .execute();
  ...
  const project_wallet = await db
    .select()
    .from(ProjectWalletTable)
    .where(eq(ProjectWalletTable.id_projek, id_projek))
    .execute();
  if (project_wallet.length === 0) {
    await db
      .insert(ProjectWalletTable)
      .values({
        id_projek,
        saldo: totalHargaToken,
        dana_terkumpul: totalHargaToken,
      })
      .execute();
  } else {
    await db
      ...
      .set({
        saldo: project_wallet[0].saldo + totalHargaToken,
        dana_terkumpul: project_wallet[0].dana_terkumpul + totalHargaToken,
      })
      .where(eq(ProjectWalletTable.id_projek, id_projek))
      .execute();
  }
  ...
  console.log("Checking project tokens status:", {
    ...
    unsoldTokens: projectTokens.filter((token: any) => token.idUser === "0")
      .length,
  ...
  const unsoldTokensCount = finalProjectTokens.filter(
    (token: any) => token.idUser === "0"
  ).length;
  ...
  if (unsoldTokensCount === 0) {
    ...
    await db
      ...
      .values({
        ...
        keterangan:
          "Penggalangan penyertaan modal untuk proyek anda berhasil memenuhi target dan sedang berjalan.",
        status: "SUCCESS",
      })
      .execute();

    const transaction = await contract.addTransaction(
      id_user,
      user[0].nama,
      id_projek,
      project[0].judul,
  ...
  return {
    message:
      unsoldTokensCount === 0
        ? "Token successfully purchased and project is now running"
...
async function checkAndUpdateProjectStatus(id_projek: string) {
  const contract = await getContract();
  const projectTokens = await contract.getTokenByProjectId(id_projek);
  const unsoldTokensCount = projectTokens.filter(
    (token: any) => token.idUser === "0"
  ).length;
...
export async function tokenUsageDetailsByIdUser(id_user: string) {
  try {
    ...
    const userTokens = allTokens.filter(
      (token: any) => token.idUser === id_user
    );
    ...
    for (const token of userTokens) {
      ...
      if (!projectTokenMap.has(projectId)) {
        ...
        if (project.length > 0) {
          ...
          const projectChartTokens = chartTokens.filter(
            (token: any) => token.idProjek === projectId
          );
          ...
          if (projectChartTokens.length > 0) {
            const latestChartToken =
              await contract.getLatestChartTokenByUserIdAndProjectId(
                id_user,
                projectId
              );
            const totalNominalTokens = await contract.getTotalNominalToken(
              id_user,
              projectId
            );
            ...
            percentage =
              totalNominalTokens.toString() === "0"
                ? 0
                : (Number(latestChartToken.nominal) /
                    Number(totalNominalTokens)) *
                  100;
          }
