...
export async function createProjectReport(projectReport: any) {
  ...
  const lastProjectReport = await db
    ...
    .where(eq(ProjectReportTable.id_projek, projectReport.id_projek))
  ...
  if (lastProjectReport.length === 0) {
    ...
    if (monthsSinceProjectCreated < reportFrequency) {
      throw new Error(
        `You can upload the report after ${
          reportFrequency - monthsSinceProjectCreated
        } months.`
      );
    }
    ...
    return {
      ...
      message: "First report uploaded successfully.",
    };
  }
  ...
  if (currentStatusIndex !== -1 && currentStatusIndex < status.length - 1) {
    const nextStatus = status[currentStatusIndex + 1] as
      | "BERJALAN"
      | "BERJALAN SIKLUS 2"
      | "BERJALAN SIKLUS 3"
      | "BERJALAN SIKLUS 4"
...
export async function getAllProjectReport(search?: string) {
  try {
    const query = db
      .select({
        user: UserTable,
      ...
      .innerJoin(
        ProjectTable,
        eq(ProjectTable.id, ProjectReportTable.id_projek)
      )
    ...
    throw new Error(
      "Could not retrieve project reports. Please try again later."
    );
  }
}
...
export async function getProjectReportById(id: string) {
  const projectReport = await db
    .select()
    .from(ProjectReportTable)
    .where(eq(ProjectReportTable.id, id))
    .execute();
...
export async function getProjectReportByProjectId(id: string, jenis?: any) {
  ...
  if (lastProjectReport.length === 0) {
    return {
      canUploadReport: true,
      message: "No reports yet, you can upload the first report.",
    };
  }
